...
def get_quarterly_targets(request):
    """Return quarterly targets and weekly progress for the given year/quarter."""
    from ..models import QuarterlyTarget, InspectorTarget, FoodSafetyAgencyInspection, InspectionGroup, InspectionFee
    ...
    q_start_month = (req_quarter - 1) * 3 + 1
    q_start = date(req_year, q_start_month, 1)
    if req_quarter == 4:
        q_end = date(req_year, 12, 31)
    else:
        q_end = date(req_year, q_start_month + 3, 1) - timedelta(days=1)

    # Build non-inspector exclusion set
    from django.contrib.auth import get_user_model
    _User = get_user_model()
    non_inspector_names = set(
        _User.objects.exclude(role='inspector').values_list('first_name', flat=True)
    ...
    # Load quarterly targets (fall back to InspectorTarget defaults)
    quarterly_targets = {}
    for qt in QuarterlyTarget.objects.filter(year=req_year, quarter=req_quarter):
        quarterly_targets[qt.inspector_name] = {
            ...
            'notes': qt.notes or '',
            'weekly': qt.weekly_targets(),
            'total_quarterly_cost': qt.total_quarterly_cost,
        }

    # Defaults from InspectorTarget for inspectors without quarterly targets
    default_targets = {}
    for t in InspectorTarget.objects.all():
        default_targets[t.inspector_name] = {
            'eggs': t.eggs, 'poultry': t.poultry, 'raw': t.raw, 'pmp': t.pmp,
            'raw_samples': t.raw_samples, 'pmp_samples': t.pmp_samples,
            'total_samples': t.total_samples,
        }

    # Fee rates for revenue calculation
    fee_rates = {}
    try:
        for fee in InspectionFee.objects.all():
            fee_rates[fee.fee_code] = float(fee.rate)
    ...
    # Actual data for the quarter (inspections)
    qs = FoodSafetyAgencyInspection.objects.filter(
        date_of_inspection__gte=q_start, date_of_inspection__lte=q_end
    ).exclude(_excl)

    group_qs = InspectionGroup.objects.filter(
        date_of_inspection__gte=q_start, date_of_inspection__lte=q_end
    ).exclude(_excl)

    # Per-inspector actuals: commodity counts
    commodity_actuals = {}
    for row in qs.values('inspector_name', 'commodity').annotate(count=Count('id')):
        name = row['inspector_name']
        if name not in commodity_actuals:
            commodity_actuals[name] = {}
        commodity_actuals[name][row['commodity'].upper() if row['commodity'] else 'OTHER'] = row['count']

    # Per-inspector actuals: samples by commodity
    sample_actuals = {}
    for row in qs.filter(is_sample_taken=True).values('inspector_name', 'commodity').annotate(count=Count('id')):
        name = row['inspector_name']
        if name not in sample_actuals:
            sample_actuals[name] = {}
        sample_actuals[name][row['commodity'].upper() if row['commodity'] else 'OTHER'] = row['count']

    # Per-inspector: hours, km from InspectionGroup
    group_actuals = {}
    for row in group_qs.exclude(_excl).values('inspector_name').annotate(
        total_hours=Sum('hours'), total_km=Sum('km_traveled')
    ):
        group_actuals[row['inspector_name']] = {
            'total_hours': float(row['total_hours'] or 0),
            'total_km': float(row['total_km'] or 0),
        }

    # Per-inspector total samples
    total_sample_actuals = {}
    for row in qs.filter(is_sample_taken=True).values('inspector_name').annotate(count=Count('id')):
        total_sample_actuals[row['inspector_name']] = row['count']

    # Weekly actuals (by ISO week) for weekly breakdown
    from django.db.models.functions import TruncWeek
    weekly_commodity = list(qs.exclude(
        Q(commodity__isnull=True) | Q(commodity='')
    ).annotate(
        week=TruncWeek('date_of_inspection')
    ).values('week', 'inspector_name', 'commodity').annotate(
        count=Count('id'),
        samples=Count('id', filter=Q(is_sample_taken=True)),
    ).order_by('week', 'inspector_name'))

    weekly_financials = list(group_qs.exclude(_excl).annotate(
        week=TruncWeek('date_of_inspection')
    ).values('week', 'inspector_name').annotate(
        total_hours=Sum('hours'),
        total_km=Sum('km_traveled'),
    ).order_by('week', 'inspector_name'))

    # Build all known inspector names
    all_inspectors = sorted(set(
        list(commodity_actuals.keys()) +
        list(quarterly_targets.keys()) +
        list(default_targets.keys()) +
        list(group_actuals.keys())
    ))

    # Build per-inspector summary
    inspector_summaries = []
    for name in all_inspectors:
        qt = quarterly_targets.get(name)
        dt = default_targets.get(name, {})
        ca = commodity_actuals.get(name, {})
        sa = sample_actuals.get(name, {})
        ga = group_actuals.get(name, {})

        # Use quarterly target if set, else default
        targets = qt if qt else {
            'eggs': dt.get('eggs', 51), 'poultry': dt.get('poultry', 59),
            'raw': dt.get('raw', 63), 'pmp': dt.get('pmp', 54),
            'raw_samples': dt.get('raw_samples', 58), 'pmp_samples': dt.get('pmp_samples', 12),
            'total_samples': dt.get('total_samples', 70),
            'monthly_salary': 0, 'quarterly_revenue_target': 0,
            'monthly_vehicle_cost': 0, 'monthly_other_costs': 0,
            'notes': '', 'weekly': None, 'total_quarterly_cost': 0,
        }

        # Calculate weekly targets if not from QuarterlyTarget
        if not targets.get('weekly'):
            weeks = 13
            targets['weekly'] = {
                'eggs': round(targets['eggs'] / weeks, 1),
                'poultry': round(targets['poultry'] / weeks, 1),
                'raw': round(targets['raw'] / weeks, 1),
                'pmp': round(targets['pmp'] / weeks, 1),
                'raw_samples': round(targets['raw_samples'] / weeks, 1),
                'pmp_samples': round(targets['pmp_samples'] / weeks, 1),
                'total_samples': round(targets['total_samples'] / weeks, 1),
                'weekly_revenue_target': 0,
                'weekly_cost': 0,
            }

        eggs_actual = ca.get('EGGS', 0) + ca.get('EGG', 0)
        hours = ga.get('total_hours', 0)
        km = ga.get('total_km', 0)
        samples_total = total_sample_actuals.get(name, 0)

        # Revenue calculation
        rev_hours = round(hours * hourly_rate, 2)
        rev_km = round(km * km_rate, 2)
        rev_samples = round(samples_total * sample_rate, 2)
        total_revenue = round(rev_hours + rev_km + rev_samples, 2)

        # Cost & profitability
        total_cost = targets.get('total_quarterly_cost', 0)
        ...
        if today > q_end:
            ...
            weeks_elapsed = max(1, ((today - q_start).days // 7) + 1)

        # Projected revenue (annualized from current pace)
        if weeks_elapsed > 0:
            weekly_revenue_pace = total_revenue / weeks_elapsed
            projected_revenue = round(weekly_revenue_pace * 13, 2)
        else:
            projected_revenue = 0
        ...
        inspector_summaries.append({
            ...
            'actuals': {
                'eggs': eggs_actual,
                'poultry': ca.get('POULTRY', 0),
                'raw': ca.get('RAW', 0),
                'pmp': ca.get('PMP', 0),
                'raw_samples': sa.get('RAW', 0),
                'pmp_samples': sa.get('PMP', 0),
            ...
            'financials': {
                ...
                'weekly_revenue_pace': round(weekly_revenue_pace, 2) if weeks_elapsed > 0 else 0,
            },
            'has_quarterly_target': name in quarterly_targets,
        })
    ...
    return JsonResponse({
        ...
        'weeks_elapsed': weeks_elapsed if today >= q_start else 0,
        'weeks_total': 13,
        'inspectors': inspector_summaries,
        'weekly_commodity': weekly_commodity,
        'weekly_financials': weekly_financials,
        'fee_rates': {'hourly': hourly_rate, 'km': km_rate, 'sample': sample_rate},
...
def save_quarterly_target(request):
    """Create or update a quarterly target for an inspector."""
    import json
    ...
    try:
        ...
        if not name or not year or quarter not in (1, 2, 3, 4):
            return JsonResponse({'error': 'inspector_name, year, and quarter (1-4) are required'}, status=400)
